...
$common = @('/nologo', '/optimize+', '/platform:x64', '/r:System.Windows.Forms.dll', '/r:System.Drawing.dll', '/r:System.Core.dll', '/r:System.Web.Extensions.dll', '/r:System.Management.dll')
...
# Needs administrator rights: moves the keyboard-key driver onto the remotes only (see DriverScope.cs).
& $csc @common /target:exe "/out:$bin\airdeck-driver.exe" "$src\Json.cs" "$src\DriverScope.cs" "$src\DriverSetup.cs"
if ($LASTEXITCODE) { throw 'airdeck-driver build failed' }
...
& $csc @common "/win32icon:$icon" "/r:$wpf\UIAutomationClient.dll" "/r:$wpf\UIAutomationTypes.dll" "/r:$wpf\WindowsBase.dll" /target:winexe "/out:$bin\airdeck.exe" `
    "$src\RawInput.cs" "$src\Json.cs" "$src\Airdeck.cs" "$src\AirdeckApp.cs" "$src\Interception.cs" "$src\Targets.cs" "$src\Screens.cs" "$src\WebServer.cs" "$src\DriverScope.cs"
...
Write-Host "Built airdeck.exe, airdeck-mapper.exe, airdeck-driver.exe and hidtool.exe in $bin"
